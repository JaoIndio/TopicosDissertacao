...
typedef union{
  struct{
    uint8_t : 3;           ///< Bits 0-2: Reservado
    uint8_t SIEN_SMUX : 1; ///< Bit 3: Interrupção do Sistema para Operação SMUX (0: Desabilitado, 1: Habilitado)
    uint8_t : 2;           ///< Bits 4-5: Reservado
...
bool AS7341_GetSMUX();
bool readChannels(uint8_t* photoDiode, uint8_t* ADC_ID, uint8_t* ADC_count);
