...
bool AS7341_BufferConfig(){
  ...
  fifo_map_reg.FIFO_WRITE_CH0_DATA   = 1; //
  fifo_map_reg.FIFO_WRITE_CH1_DATA   = 1; //
  fifo_map_reg.FIFO_WRITE_CH2_DATA   = 1; //
  fifo_map_reg.FIFO_WRITE_CH3_DATA   = 1; //
...
bool readChannels(uint8_t* photoDiode, uint8_t* ADC_config, uint8_t* ADC_count){

  if(!AS7341_Enable()) return false;
  if(!AS7341_SetSMUX(photoDiode, ADC_config)) return false;
  if(!AS7341_EnableSpecMen()) return false;

  if(!AS7341_readMultiples(AS7341_REG_CH0_DATA_L, ADC_count, 12)) return false; // Le o canal que eu quero
  if(!AS7341_DisableSpecMen()) return false; 
