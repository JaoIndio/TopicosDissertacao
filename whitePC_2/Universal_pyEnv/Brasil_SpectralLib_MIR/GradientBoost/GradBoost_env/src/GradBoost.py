...
#
#if path_2Use=="Colab":
generalPahth=(f"/home/gedre/JoaoFelipeIndio/TopicosDissertacao/Processamento/Universal_pyEnv/Brasil_SpectralLib_MIR")
#else:
...
def optimise_GradBoost_cv(X, y, n_comp):
  ...
  losses                   =  ['huber']     #['squared_error', 'huber', 'quantile'] 
  learning_rate            =  [0.01,0.1]    #,1,10,100]                        # Layer 2                    
  n_estimators             =  [125,250,500] #[1,5,25,125,250,500]                       # Layer 3                
  subsample                =  [0.3,0.5,0.75,1] #[0.3,0.5,0.75,0.8,1]                       # Layer 4                      
  criterion                = ['friedman_mse']          # Layer 5                          
  min_samples_split        = [2] #,4,8,16,32,20,40,80]                     # Layer 6                       
  min_samples_leaf         = [1] #[1,2,4,8,16,32]                            # Layer 7  
  min_weight_fraction_leaf = [0, 0.1, 0.2, 0.4, 0.5] #[0, 0.01, 0.1, 0.2, 0.4, 0.5]              # Layer 8    
  max_depth                = [7,9]#[5,7,9]                                # Layer 9    
  max_features             = [None]#, 'sqrt', 'log2']                     # Layer 11        
  max_leaf_nodes           =  [None, 100, 1000]#[None, 10, 100, 1000]     # Layer 12                  
  ...
  while random_num < 2147483647:
   ...
   #tol                     = tol[tol_i],\
          ...
          subsample_i=0  
          n_estimators_i+=1
          #print("\t\t\t\t\t\t\t**... Grad Boost estimators-> ", n_estimators_i)
        n_estimators_i=0
        learning_rate_i+=1
        #print("\t\t\t\t\t\t**... Grad Boost learning_rate_i-> ", learning_rate_i)
      learning_rate_i=0
      #print("\t\t\t\t\t\t\t**... Grad Boost Loss MIR-> ", loss_i)
      loss_i+=1
    loss_i=0
    
    #print("**... Grad Boost Random_num MIR-> ", random_num)
    random_num+=1
