...
OVERLAP_PERCENTAGE = 99.9  # 85% overlap for ultra-smooth sliding
...
def init_blitting():
    ...
    print("✓ Blitting initialized for ultra-fast updates")
    ...
    if fft_cache['window_size'] == num_samples:
        ...
        if USE_BACKGROUND_THREAD:
            ...
            result = {
                ...
                'x_vals': fft_cache['x_vals'],
                'y_vals': fft_cache['y_vals'][np.argmin(np.abs(fft_cache['positions'] - center_position)), :],
                'mode': fft_cache['mode'],
                'actual_position': fft_cache['positions'][np.argmin(np.abs(fft_cache['positions'] - center_position))],
                'x_label': fft_cache['mode'].split()[0],
                'peak_x': fft_cache['x_vals'][np.argmax(fft_cache['y_vals'][np.argmin(np.abs(fft_cache['positions'] - center_position)), :])],
                'peak_y': np.max(fft_cache['y_vals'][np.argmin(np.abs(fft_cache['positions'] - center_position)), :]),
                'n_windows': len(fft_cache['positions'])
            }
            update_fft_display_blit(result)
