...
NOW WITH MANUAL WINDOW SIZE ENTRY!
...
OVERLAP_PERCENTAGE = 85  # 85% overlap for ultra-smooth sliding
...
data_values = []
window_size = None
manual_window_center = None  # Track center position for manual window
...
# Manual window size entry controls - NEW!
...
# Variable to store the processed region indicator
...
def load_csv(event):
    ...
    ax1.set_title(f'Data from {csv_filename} ({len(data_values)} points) | Use zoom tools or box zoom', fontsize=14)
    ax1.legend()
    ...
    if toolbar:
        toolbar.zoom()  # Enable zoom mode by default
    
    status_text.set_text(f"✓ Loaded {len(data_values)} points | Zoom to region of interest, then select window & FFT")
    
    print("\t\t\tLoad csv fig canva draw")
    fig.canvas.draw_idle()


def set_manual_window(event):
    """Set window size manually from text box input - NEW FUNCTION!"""
    ...
    try:
        # Get window size from text box
        manual_size = int(window_size_box.text.strip())
        
        if manual_size < 4:
            status_text.set_text(f"❌ Window size must be at least 4 (got {manual_size})")
            return
        
        if manual_size > len(data_values):
            status_text.set_text(f"❌ Window size ({manual_size}) exceeds data length ({len(data_values)})")
            return
        ...
        # Set window at center of data (or use last manual position)
        #if manual_window_center is None or manual_window_center < manual_size/2 or manual_window_center > len(data_values) - manual_size/2:
        #    manual_window_center = len(data_values) / 2
        
        # Calculate window boundaries
        selected_start = manual_window_center - manual_size
        selected_end = manual_window_center + manual_size
        
        # Ensure boundaries are within data limits
        if selected_start < 0:
            selected_start = 0
            selected_end = manual_size
            manual_window_center = manual_size / 2
        elif selected_end > len(data_values):
            selected_end = len(data_values)
            selected_start = selected_end - manual_size
            manual_window_center = selected_start + manual_size / 2
        ...
        selected_rect = plt.Rectangle(
            ...
            linewidth=0.125,
            edgecolor='blue',  # Blue for manual selection
            facecolor='blue',
            alpha=0.2
        )
        ...
        selection_text = ax1.text(
            selected_start + (selected_end - selected_start) / 2,
            ax1.get_ylim()[1] * 0.98,
            f"Manual Window: {manual_size} samples",
            color='blue',
            ha='center',
            fontsize=10,
            weight='bold',
        ...
        # Update span selector to match
        span.set_active(True)
        span.extents = (selected_start, selected_end)
        
        status_text.set_text(f"✓ Manual window set: {manual_size} samples | Press FFT to compute")
        
        print("\t\t\tSet manual window fig canva draw")
        fig.canvas.draw_idle()
        
        print(f"Manual window set: size={manual_size}, start={selected_start:.0f}, end={selected_end:.0f}")
        
    except ValueError:
        status_text.set_text(f"❌ Invalid window size: '{window_size_box.text.strip()}' (must be integer)")
        return
...
def compute_all_ffts_parallel(data, win_size, mode, sampling_interval):
    ...
    print("\t\t\tConpute all ffts fig canva draw")
    fig.canvas.draw_idle()
...
def data_preparation_worker():
    ...
    global thread_running, g_peak_y
    ...
    while thread_running:
        try:
            ...
            center_position = update_queue.get(timeout=0.750)
...
def update_fft_display_blit(result_data):
    ...
    print("update_fft_display_blit: if 1")

    if not blit_cache['enabled'] or blit_cache['background'] is None:
        if not blit_cache['enabled']:
          print("update_fft_display_blit: enabled")
        if blit_cache['background'] is None:
          print("update_fft_display_blit: background is None")
        return
    
    print("update_fft_display_blit: restore")
    # Restore background
    fig.canvas.restore_region(blit_cache['background'])
    
    # Update line data
    print("update_fft_display_blit, Update line")
    blit_cache['line'].set_data(result_data['x_vals'], result_data['y_vals'])
    
    # Update axes limits if needed
    print("update_fft_display_blit, set x y limits")
    ax2.set_xlim(min(result_data['x_vals']), max(result_data['x_vals']))
    ax2.set_ylim(0, result_data['peak_y'] * 1.1)
    print("update_fft_display_blit, peak Y: ",result_data['peak_y'] )
    ...
    if result_data['fwhm'] is not None:
        ...
        print("update_fft_display_blit, Update FWHM horizontal line")
        blit_cache['fwhm_line'].set_segments([[(result_data['fwhm_left'], half_max), 
                                                 (result_data['fwhm_right'], half_max)]])
        # Update FWHM markers
        print("update_fft_display_blit, Update FWHM markers")
        blit_cache['fwhm_markers'].set_data([result_data['fwhm_left'], result_data['fwhm_right']], 
                                             [half_max, half_max])
        
        # Update peak text with FWHM
        text_x = result_data['peak_x'] +100 if result_data['peak_x'] < np.median(result_data['x_vals']) else result_data['peak_x'] +100
        blit_cache['peak_text'].set_position((text_x, result_data['peak_y'] * 0.80))
    ...
    print("update_fft_display_blit, redraw line")
    ax2.draw_artist(blit_cache['line'])
    print("update_fft_display_blit, redraw peak line")
    ax2.draw_artist(blit_cache['peak_line'])
    print("update_fft_display_blit, redraw fwhm line")
    ax2.draw_artist(blit_cache['fwhm_line'])
    print("update_fft_display_blit, redraw fwhm markers")
    ax2.draw_artist(blit_cache['fwhm_markers'])
    print("update_fft_display_blit, redraw peak text")
    ax2.draw_artist(blit_cache['peak_text'])
    
    # Blit the updates
    print("update_fft_display_blit, blit")
    fig.canvas.blit(ax2.bbox)
    fig.canvas.flush_events()
    print("update_fft_display_blit, fig.canvas.flush_events()")
...
def update_fft_display_full(center_position):
    ...
    if len(x_vals) > 0 and len(y_vals) > 0:
        ...
        text_x = peak_x * 1.50 if peak_x < np.median(x_vals) else peak_x * 1.50
        if fwhm is not None:
    ...
    print("\t\t\tupdate_fft_display_full fig canva draw")
    fig.canvas.draw_idle()
...
def zoom_reset(event):
    ...
    if data_values:
        ...
        status_text.set_text(f"✓ Zoom reset - showing all {len(data_values)} points")
        fig.canvas.draw_idle()


def run_fft(event):
    """Pre-compute all FFTs for the selected window size - ONLY FOR VISIBLE REGION."""
    global data_values, fft_cache, window_size, manual_window_center, processed_region_patch
    ...
    # Store manual window center if it was set manually
    if selection_text and 'Manual' in selection_text.get_text():
        manual_window_center = (selected_start + selected_end) / 2
    ...
    if visible_length < window_size:
        print(f"Visible region ({visible_length} points) smaller than window size ({window_size} points)")
        status_text.set_text(f"❌ Visible region too small for window size")
        return
    
    # Remove previous processed region indicator
    global processed_region_patch
    if processed_region_patch:
        processed_region_patch.remove()
        processed_region_patch = None
    
    # Add visual indicator of processed region (green shaded area)
    processed_region_patch = plt.Rectangle(
        ...
        facecolor='green',
        alpha=0.05,
        zorder=-1  # Behind the data
    )
    ax1.add_patch(processed_region_patch)
    
    print(f"\n{'='*60}")
    print(f"Computing FFTs for VISIBLE region only:")
    print(f"  Full data: {len(data_values)} points")
    print(f"  Visible region: {visible_length} points (indices {visible_start}-{visible_end})")
    print(f"  Window size: {window_size} points")
    print(f"{'='*60}\n")
    
    mode = domain_selector.value_selected
    
    # Compute FFTs only for visible data
    x_axis, y_matrix, positions_relative = compute_all_ffts_parallel(
        np.array(visible_data), 
        window_size, 
        mode, 
        SAMPLING_INTERVAL_NM
    )
    
    # Adjust positions to account for offset from full data
    positions_absolute = positions_relative + visible_start
    
    # Store in cache
    fft_cache['x_vals'] = x_axis
    fft_cache['y_vals'] = y_matrix
    fft_cache['positions'] = positions_absolute  # Use absolute positions
    fft_cache['window_size'] = window_size
    fft_cache['mode'] = mode
    fft_cache['computed'] = True
    fft_cache['visible_start'] = visible_start  # Store for reference
    fft_cache['visible_end'] = visible_end
    ...
    print("\t\t\trun fft: fig canva draw")
    fig.canvas.draw_idle()
    
    status_text.set_text(f"✓ {len(positions_absolute)} FFTs ready (visible region) | Drag window for real-time updates")
    
    # Display first FFT
    if len(positions_absolute) > 0:
        update_fft_display_full(positions_absolute[0])
    
    print(f"FFT cache ready with {len(positions_absolute)} windows (visible region only)")
    print(f"Memory usage: ~{(y_matrix.nbytes / 1024 / 1024):.1f} MB")
    print(f"Computation savings: {100*(1-visible_length/len(data_values)):.1f}% of data skipped")
...
def onselect(xmin, xmax):
    ...
    selected_start = xmin
    selected_end = xmax
    manual_window_center = (xmax-xmin)/2 + xmin  # Reset manual center when using span selector
    print("onselect: manual_window_center ", manual_window_center)
    print("onselect: xmin ", selected_start)
    print("onselect: xmax ", selected_end)
    ...
    print("\t\t\tOn select fig canva draw")
    fig.canvas.draw_idle()
...
def on_span_motion(xmin, xmax):
    ...
    if fft_cache['window_size'] == num_samples:
        ...
        if USE_BACKGROUND_THREAD:
            print("on_span_motion USE_BACKGROUND THREAD ")
            try:
                update_queue.get_nowait()  # Clear old request
            except:
                pass
            print("on_span_motion put ")
        ...
        #print("\t\t\tOn span motion fig canva draw")
        #fig.canvas.draw_idle()


def reset(event):
    """Reset the plots and selections."""
    global selected_rect, selected_start, selected_end, selection_text, data_values, fft_cache, blit_cache, manual_window_center, processed_region_patch
...
ax1.set_xlabel('Sample Index', fontsize=12)
ax1.set_ylabel('Intensity', fontsize=12)
ax1.set_title('Load data to begin | Use matplotlib zoom tools', fontsize=14)
ax1.grid(True, alpha=0.3)
...
print("NEW: Manual window size entry available!")
print("NEW: FFT computation only for VISIBLE region!")

fft_button.on_clicked(run_fft)
reset_button.on_clicked(reset)
load_button.on_clicked(load_csv)
set_window_button.on_clicked(set_manual_window)  # NEW!
zoom_reset_button.on_clicked(zoom_reset)  # NEW!
