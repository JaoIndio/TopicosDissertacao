...
- Process usage: 11 workers (92% utilization) - TRUE multiprocessing, not threads!
- Overlap: 85% for ultra-smooth real-time FFT display

PERFORMANCE NOTE:
Uses ProcessPoolExecutor (not threads) to bypass Python's GIL and fully utilize all CPU cores.
"""
...
OVERLAP_PERCENTAGE = 99.95  # 85% overlap for ultra-smooth sliding (you have the power!)
MAX_WORKERS = 11  # Use 11 of 12 threads (leave 1 for OS/GUI)

# Memory and performance optimizations for 16GB RAM + Ryzen 5 PRO 5675U
BUFFER_SIZE_MULTIPLIER = 3  # Larger buffers for smooth operation
USE_FLOAT32 = True  # Use float32 for FFT cache (saves memory, still precise)
...
def compute_all_ffts_parallel(data, win_size, mode, sampling_interval):
    ...
    # Parallel FFT computation with TRUE multiprocessing
    results = {}
...
def update_fft_display(center_position):
    ...
    if len(x_vals) > 0 and len(y_vals) > 0:
        ...
        # Calculate FWHM
        fwhm, fwhm_left, fwhm_right = calculate_fwhm(x_vals, y_vals, peak_idx)
        
        ax2.axvline(peak_x, color='red', linestyle='--', alpha=0.5, linewidth=0.5)
        
        text_x = peak_x * 1.02 if peak_x < np.median(x_vals) else peak_x * 0.98
        
        if fwhm is not None:
          # Display peak and FWHM
          ax2.text(text_x+30, peak_y * 0.80, 
                    f'Peak: {peak_x:.1f} nm\nFWHM: {fwhm:.2f} nm', 
...
def on_span_motion(xmin, xmax):
    """Handle real-time updates while dragging the span selector."""
    global selected_rect, selection_text
    
    # Only update if FFTs are pre-computed
    if not fft_cache['computed']:
        return
    
    num_samples = int(abs(xmax - xmin))
    
    # Only update FFT if window size matches pre-computed size
    if fft_cache['window_size'] == num_samples:
        center_position = (xmin + xmax) / 2
        update_fft_display(center_position)
        
        # Update the selection text position during drag
        if selection_text:
...
print("Using ProcessPoolExecutor for TRUE multi-core parallelism!")
fft_button.on_clicked(run_fft)
reset_button.on_clicked(reset)
load_button.on_clicked(load_csv)

# Setup span selector for interactive region selection with motion callback
span = SpanSelector(
    ...
    onmove_callback=on_span_motion  # THIS IS THE KEY: real-time updates while dragging!
)
