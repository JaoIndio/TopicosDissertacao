...
u0.gauss_beam(A=1, w0=4*um, r0=(0*um, 0*um), z0=0, theta=0) # nao sei qual o valor real
#outPutLight = u0.RS(z=140*um)
#outPutLight.draw(kind='intensity', normalize=True)

u1 = Scalar_source_XY(x, y, wavelength)
u1.gauss_beam(A=1, w0=6*um, r0=(0*um, 0*um), z0=0, theta=0) # nao sei qual o valor real

u2 = Scalar_source_XY(x, y, wavelength)
u2.gauss_beam(A=1, w0=1*mm, r0=(0*um, 0*um), z0=0, theta=0) # nao sei qual o valor real
...
correlation_length = 1*um  # Correlation length in grid points (e.g., ~3.9 um)
...
#phase_mask /= np.sqrt(np.mean(np.abs(phase_mask)**2))  # Normalize energy
#
...
outPutLight = u0.RS(z=30*um)
outPutLight.draw(kind='intensity', normalize=True)

outPutLight1 = u1.RS(z=30*um)
outPutLight1.draw(kind='intensity', normalize=True )

outPutLight2 = u1.RS(z=30*um)
outPutLight2.draw(kind='intensity', normalize=True)


#
#ted Diode Laser (using gauss_beam and defining its spacial incoherence, if nedded, I'm not sure if applies for it or not)
