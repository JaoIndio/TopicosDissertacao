...
def optimise_SVR_cv(X, y, n_comp):
  ...
  Gamma_BestR2=0
  Gamma_BestRPD=0
  bestFit_file=(f"{rbf_PCA}bestFit_SVR.csv")
  ...
  print("\
  RBF PCA Backup  Best Fit Analyse\n\
  \t\tMaxRPD:           ",AllData[0],"\n\
  \t\tMaxR2:           ",AllData[1],"\n\
  ...
  for varia in pca_variance:
    ...
    #print("\n\n\n================")
     # X_spectral_aux=pd.DataFrame(data=X[i,:])
  ...
  while random_num < 2147483647:
    ...
    #print("RBF MIR PCA ----------> random", random_num)
    while C<100000:
      #print("RBF MIR PCA C", C)
      while Gamma<100000:
        #print("RBF MIR PCA Gamma", Gamma)
        ...
        for i in pca_control:
          ...
          if rpd>max_rpd:
            ...
            with open(bestFit_file, 'a') as file:
              data=(f"max_rpd,max_r2,R2_rdn,RPD_rdn,C_R2,Gamma_R2,C_RPD,Gamma_RPD,PCA Variancia,random_numbers\n\
                {max_rpd},{max_r2},\
                {best_randomR2},{best_randomRpd},\
                {C_BestR2},{Gamma_BestR2},\
                {C_BestRPD},{Gamma_BestRPD},{varia},\
                {random_num}\n\
                ==============================================================\n\n")
              file.write(data)
              file.close()
    ...
    C=0.0001 
    if control_print==25:
      print("**... RBF PCA MIR -> ", random_num)
      with open(Bkp_file, 'a') as file:
        data=(f"C,Gamma,PCA Variancia,random_num\n\
        {C},{Gamma},{varia},{random_num}\n\
        ******************************************\n\n")
        file.write(data)
        file.close()
      control_print=0
